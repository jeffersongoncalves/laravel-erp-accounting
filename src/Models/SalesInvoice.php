 ...
 * @property-read Account $debitTo
 * @property-read Collection<int, SalesInvoiceItem> $items
 * @property-read Collection<int, SalesInvoiceTax> $taxes
 */
class SalesInvoice extends Model implements PostsToLedger, SubmittableDocument
{
    use HasCompany;
    use HasFactory;
    use HasNamingSeries;
    ...
    {
        return (config('erp-accounting.table_prefix') ?? '').'sales_invoices';
    }

    public function debitTo(): BelongsTo
    {
        return $this->belongsTo(ModelResolver::account(), 'debit_to_id');
    }

    public function items(): HasMany
    {
        return $this->hasMany(ModelResolver::salesInvoiceItem(), 'sales_invoice_id');
    }

    public function taxes(): HasMany
    {
        return $this->hasMany(ModelResolver::salesInvoiceTax(), 'sales_invoice_id');
    }
    ...
    {
        $entries = [[
            'account_id' => $this->debit_to_id,
            'debit' => $this->grand_total,
            'credit' => 0,
            'party_type' => $this->party_type,
            'party_id' => $this->party_id,
            'against' => $this->customer_name,
        ]];
